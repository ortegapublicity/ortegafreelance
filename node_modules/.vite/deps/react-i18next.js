...
var warnOnce = (i18n, code, msg, rest) => {
  if (isString(msg) && alreadyWarned[msg])
    return;
  if (isString(msg))
  ...
  const mapAST = (reactNode, astNode, rootReactNode) => {
    ...
    return astNodes.reduce((mem, node, i2) => {
      ...
      if (node.type === "tag") {
        ...
        if (!tmp && knownComponentsMap)
          tmp = knownComponentsMap[node.name];
...
var useTranslation = (ns, props = {}) => {
  ...
  const subscribe = (0, import_react7.useCallback)((callback) => {
    ...
    if (bindI18n)
      i18n.on(bindI18n, callback);
    if (bindI18nStore)
      i18n.store.on(bindI18nStore, callback);
    return () => {
      if (bindI18n)
        bindI18n.split(" ").forEach((e2) => i18n.off(e2, callback));
      if (bindI18nStore)
        bindI18nStore.split(" ").forEach((e2) => i18n.store.off(e2, callback));
    };
  }, [i18n, i18nOptions]);
...
var useSSR = (initialI18nStore, initialLanguage, props = {}) => {
  ...
  const i18n = i18nFromProps || i18nFromContext || getI18n();
  if ((_a = i18n.options) == null ? void 0 : _a.isClone)
    return;
